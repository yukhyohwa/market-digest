...
def send_report_email(report_path):
    ...
    # Extract only the date from filename
    import re
    filename = os.path.basename(report_path)
    date_match = re.search(r'\d{4}-\d{2}-\d{2}', filename)
    report_tag = date_match.group(0) if date_match else "today"
    
    subject_title = filename.replace(".md", "")
    ...
    <style>
        ...
        img.report-chart {{ width: 66.67%; max-width: 100%; height: auto; display: block; }}
    ...
    <body>
        {html_content}
        <div class="footer">
            Generated by AI Market Analyst | Data retrieved at {report_tag}
        </div>
    </body>
    </html>
    """
